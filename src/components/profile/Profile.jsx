...
const Profile = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const response = await axios.get(`https://ecomback-sli5.onrender.com/${id}`, {
          headers: {
  ...
  useEffect(() => {
    const Bookings = async () => {
      try {
        const response = await axios.get(`https://ecomback-sli5.onrender.com/Booking/user/${id}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  const cancel=async(id)=>{
    const ID=id
    try {
      const resp=await axios.delete(`https://ecomback-sli5.onrender.com/booking/${ID}`)
